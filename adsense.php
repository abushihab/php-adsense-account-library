...
class AdSense {
    ...
    function connect($username, $password){
        ...
        // did we login ?
        return preg_match('~<a href="/adsense/signout">~i',  $content);
    }
     ...
     * Use get_report_id_from_name to get id by report name.
     *
     * @param int $id
    ...
    function report($report_id){
        ...
        if (preg_match('/var\s+reportTable\s+=\s+new\s+AsyncReportTable\(([^\)]+)\);/si', $content, $matches)) {
            ...
            curl_setopt($this->curl, CURLOPT_URL,
                'https://www.google.com/adsense/report/online-stored-reporttable?storedReportId='.$params[0]
                 .'&reportUri='.str_replace('?', '%3F', str_replace('/', '%2F', str_replace('\x', '%', $params[2])))
                 .'&formId='.$params[3]
                 .'&title='.str_replace('+', '%20', urlencode($params[1])) . '&waitTime0'
            );
            do {
                $content = curl_exec($this->curl);
            ...
            if (preg_match_all('/<tr\s+class="(odd|even)\s+datarow">\s*' .
                ...
                foreach ($matches[2] as $k => $v) {
                    $result[$k] = array(
                        'channel' => preg_replace('/<.+>/', '', $v),
                        'impressions' => $matches[3][$k],
                        'clicks' => $matches[4][$k],
                        'ctr' => $matches[5][$k],
                        'ecpm' => $matches[6][$k],
                        'earnings' => $matches[7][$k]
                    );
                }
            }
        }
    ...
    function quick_report($url){
        ...
        if (preg_match('/var\s+reportTable\s+=\s+new\s+AsyncReportTable\(([^\)]+)\);/si', $content, $matches)) {
            ...
            $reportUri = strtr($params[2], array(
                '&' => '%26',
                '=' => '%3D',
                '?' => '%3F',
                '/' => '%2F'
            ...
            curl_setopt($this->curl, CURLOPT_URL,
                ...
                .'&reportUri='.$reportUri
                .'&formId='.$params[3]
                .'&title='.str_replace('+', '%20', urlencode($params[1]))
                .'&waitTime0'
            );
            $tr = 0;
            do {
            ...
            while (empty($content) || $content === true);

            if (preg_match_all(
                '~<tr\s+class="(odd|even)\s+datarow">\s*'
                .'<td[^>]*>(.*?)<\/td>\s*'
                .'<td[^>]*>(.*?)<\/td>\s*'
                .'<td[^>]*>(.*?)<\/td>\s*'
                .'<td[^>]*>(.*?)<\/td>\s*'
                ...
                foreach ($matches[3] as $k => $v) {
                    ...
                    $result[$day][$n] = array(
                        'channel' => preg_replace('/\s*<[^>]+>\s*/', '', $v),
                        'impressions' => $matches[4][$k],
                        'clicks' => $matches[5][$k],
                        'ctr' => $matches[6][$k],
                        'ecpm' => $matches[7][$k],
                        'earnings' => $matches[8][$k]
                    );
